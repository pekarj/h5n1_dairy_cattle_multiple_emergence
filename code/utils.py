...
def toYearFraction(datestring: str) -> float:
    """Convert 'YYYY-MM-DD' to decimal year (y + fraction), day-level precision."""
    from datetime import datetime, date
    d = datetime.strptime(datestring, "%Y-%m-%d").date()
    y0 = date(d.year, 1, 1)
    y1 = date(d.year + 1, 1, 1)
    return d.year + (d - y0) / (y1 - y0)  # timedelta / timedelta -> float

def toDatestring(year_fraction: float) -> str:
    """Convert decimal year (y + fraction) back to 'YYYY-MM-DD'."""
    import math
    from datetime import date, timedelta

    year = math.floor(year_fraction)
    frac = year_fraction - year

    # Handle exact 1.0 edge cases robustly
    if math.isclose(frac, 1.0, rel_tol=0.0, abs_tol=1e-12):
        year += 1
        frac = 0.0

    y0 = date(year, 1, 1)
    y1 = date(year + 1, 1, 1)
    d = y0 + (y1 - y0) * frac  # scales by 365 or 366 correctly
    return d.isoformat()
